...
void motion(int x, int y){ ///加入mouse motion 對應的函式
    ...
    angleY [angleID] += x - oldX; /// 當我們的 mouse motion 時, x座標的對應角度
    oldX = x;
    oldY = y;
    glutPostRedisplay(); ///要重畫畫面
    ///把原來motion裡存檔的程式碼剪下，貼進 keyboard
}
    ...
    glPushMatrix();
        ...
        glPushMatrix();
            ...
            glPushMatrix(); ///(0)
                ...
                glRotatef(angleY[3], 0, 1, 0); ///(2)x軸轉
...
///void timer(int t){ ///實驗用 先註解
    ///glutTimerFunc(20, timer, t+1);///實驗用 先註解
    ///printf("現在起床:%d\n",t);///實驗用 先註解
///}
...
int main(int argc, char*argv[])
{
    ...
    ///glutTimerFunc(0, timer, 0); ///week14-1新加的///實驗用 先註解
